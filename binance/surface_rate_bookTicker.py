...
def find_arb_opportunity_surf(pair, initial_amount):
    ...
    def get_desc(step, in_c, in_a, rate, info, out_c, out_a, d):
        ...
        rate_detail = f"1 / Avg Ask = {price:.8f}" if "1/Ask" in info else f"Avg Bid = {price:.8f}"
        d_str = d.replace('_', ' ').title()
        return f"{step}. Start with {in_c} of {in_a}. Swap at rate {rate:.8f} ({rate_detail}) for {out_c} acquiring {out_a} ({in_a} * {rate:.8f} = {out_a}) -- {d_str}."
    ...
    for direction in direction_list:
        ...
        # Initialize variables for this direction pass
        contract_1 = ""
        contract_2 = ""
        contract_3 = ""
        direction_trade_1 = ""
        ...
        if direction == "forward":
            ...
            # Use bid price for the first trade for selling (Base to Quote)
            try:
                swap_1_rate = a_bid
                swap_1_info = "(Bid)"
            ...
            if a_quote == b_quote and calculated == 0:
                ...
                # We will use the ASK price because we still have the same quote coin (ETH/BTC & LTC/BTC). In this case sell the coin (ETH/BTC) that we have, and buy new base coin with the same quote coin (LTC/BTC) with ASK price (BUY).
                swap_2_rate = 1 / b_ask
                swap_2_info = "(1/Ask)"
                ...
                if b_base == c_base:
                    # Next direction will be base to quote, because it has different quote coin (LTC/BTC & LTC/ETH). So we will use BID price to sell LTC/ETH with amount of LTC/BTC.
                    try:
                        swap_4 = c_quote
            ...
            if a_quote == b_base and calculated == 0:
                try:
                    swap_3 = b_quote
                    # We will use the BID price because we have different quote coins (ETH/BTC & BTC/TRY). Divided by 1 to represent the inverse of the price, example from ETH/BTC to BTC/ETH. Then amount of ETH/BTC multiplied by 1 divide by ASK price BTC/TRY (BUY) = amount of BTC/TRY that we will get.
                    swap_2_rate = b_bid
                    swap_2_info = "(Bid)"
            ...
            if a_quote == c_quote and calculated == 0:
                ...
                if c_base == b_base:
                    try:
                        swap_4 = b_quote
                        # Next direction will be base to quote, use BID price because quote coin different.
                        swap_3_rate = b_bid
                        swap_3_info = "(Bid)"
                        acquired_coin_t3 = acquired_coin_t2 * swap_3_rate
                        direction_trade_3 = "base_to_quote"
            ...
            trade_desc_1 = get_desc(1, swap_1, starting_amount, swap_1_rate, swap_1_info, swap_2, acquired_coin_t1, direction_trade_1)
            trade_desc_2 = get_desc(2, swap_2, acquired_coin_t1, swap_2_rate, swap_2_info, swap_3, acquired_coin_t2, direction_trade_2)
            trade_desc_3 = get_desc(3, swap_3, acquired_coin_t2, swap_3_rate, swap_3_info, swap_4, acquired_coin_t3, direction_trade_3)
        ...
        if direction == "reverse" and calculated == 0:
            ...
            # Use ask price for the first trade for buying (Quote to Base)
            swap_1 = a_quote
            swap_2 = a_base
            swap_1_rate = 1 / a_ask
            swap_1_info = "(1/Ask)"
